...
class Category_Catalog_Frontend_Controller extends Catalog_Frontend_Controller {
    ...
    private function getCategory() {
         ...
         * и URL категории. При переходе на страницу категории сохраняются все фильтры,
         * которые пользователь применил к текущей категории. Кроме того, если у дочерней
         * категории все товары принадлежат одной функциональной группе, сразу включается
         * фильтр по функционалу. Чтобы при переходе в эту категорию сразу стали доступны
         * параметры подбора (без выбора единственной функциональной группы из выпадающего
         * списка)
         *
         * $categories = Array (
         *   [0] => Array (
