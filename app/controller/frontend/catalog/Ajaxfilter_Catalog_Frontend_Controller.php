...
class Ajaxfilter_Catalog_Frontend_Controller extends Catalog_Frontend_Controller {
    ...
    /**
     * результаты результат фильтрации товаров в формате JSON
     */
    private $output;
    ...
    public function request() {
        ...
        if ($group && isset($_POST['param'])) {
            ...
            if (isset($_POST['change']) && ctype_digit($_POST['change']) && $_POST['change'] > 0) {
        ...
        $params = $this->catalogFrontendModel->getGroupParams($this->params['category'], $group, $maker, $param, $change);
        ...
        $result = array('childs' => $output[0], 'fields' => $output[1], 'products' => $output[2]);
        // массив функциональных групп
        $result['options']['groups'] = $this->catalogFrontendModel->getCategoryGroups($this->params['category'], $maker);
        // массив производителей
        $result['options']['makers'] = $this->catalogFrontendModel->getCategoryMakers($this->params['category'], $group, $param);
        // массив параметров подбора
        $result['options']['params'] = $params;
        $this->output = json_encode($result);
        // file_put_contents('out.txt', $this->output);
    }
