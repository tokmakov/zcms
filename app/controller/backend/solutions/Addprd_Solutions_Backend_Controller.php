<?php
/**
 * Класс Addprd_Solutions_Backend_Controller формирует страницу с формой
 * для добавления товара в типовое решение. Получает данные от модели
 * Solutions_Backend_Model, административная часть сайта
 */
class Addprd_Solutions_Backend_Controller extends Solutions_Backend_Controller {
    ...
    protected function input() {
         ...
         * Addprd_Solutions_Backend_Controller
         */
        parent::input();
        ...
        if ($this->isPostMethod()) {
            if ($this->validateForm()) { // ошибок не было, добавление товара прошло успешно
                ...
                $this->redirect($this->solutionsBackendModel->getURL('backend/solutions/show/id/' . $this->params['parent']));
            } else { // при заполнении формы были допущены ошибки
                // перенаправляем администратора на страницу с формой для исправления ошибок
                $this->redirect($this->solutionsBackendModel->getURL('backend/solutions/addprd/parent/' . $this->params['parent']));
            }
        }
    ...
    protected function validateForm() {
        ...
        $this->solutionsBackendModel->addSolutionProduct($data);
